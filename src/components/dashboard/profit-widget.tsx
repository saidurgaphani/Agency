import { Bar, BarChart, ResponsiveContainer } from "recharts";
import { profitData } from "@/data/dashboard-mock-data";
import { cn } from "@/lib/utils";

interface ProfitWidgetProps {
  className?: string;
}

export const ProfitWidget = ({ className }: ProfitWidgetProps) => {
  return (
    <div className={cn("relative p-6 rounded-3xl bg-white/5 border border-border backdrop-blur-sm shadow-[inset_0px_1px_2px_0px_rgba(255,255,255,0.1)]", className)}>
      <div className="flex items-center justify-between mb-4">
        <div>
          <h3 className="text-foreground text-base font-semibold">Total Profit</h3>
          <p className="text-muted-foreground text-sm">Last 6 months</p>
        </div>
      </div>

      <div className="flex items-end justify-between">
        <div>
          <span className="text-foreground text-3xl font-semibold">$62.5K</span>
          <div className="flex items-center gap-1 mt-1">
            <span className="text-emerald-400 text-sm font-medium">+18.2%</span>
            <span className="text-muted-foreground text-xs">vs last period</span>
          </div>
        </div>

        <div className="h-[80px] w-[120px]">
          <ResponsiveContainer width="100%" height="100%">
            <BarChart data={profitData} barGap={4}>
              <Bar
                dataKey="value"
                fill="url(#profitGradient)"
                radius={[4, 4, 0, 0]}
              />
              <defs>
                <linearGradient id="profitGradient" x1="0" y1="0" x2="0" y2="1">
                  <stop offset="0%" stopColor="hsl(0, 0%, 100%)" />
                  <stop offset="100%" stopColor="hsl(0, 0%, 60%)" />
                </linearGradient>
              </defs>
            </BarChart>
          </ResponsiveContainer>
        </div>
      </div>
    </div>
  );
};
